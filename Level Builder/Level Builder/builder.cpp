...
bool EditLevel(char* level, int& cursorX, int& cursorY, int width, int height);
	...
	while (!done)
	{
		...
		if (input == 1)
		{
			...
			else
			{
				...
				levelFile.read(pLevel, levelHeight * levelHeight);
				levelFile.close();
...
{
	cout << "Pick a name for your level file (eg. level11.txt)";
	string levelName;
	cin >> levelName;
